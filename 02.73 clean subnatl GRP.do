...
// collapse to OECD region level
    gcollapse (sum) del_sum_pix del_sum_area sum_pix_bm sum_pix_bm_dec pol_area (mean) GRP, ///
        by(gid_1 iso3c year source)
    rename gid_1 region
	
	// remove fake zeros
	foreach i in del_sum_pix del_sum_area sum_pix_bm sum_pix_bm_dec pol_area {
	    replace `i' = . if `i' == 0
	}
