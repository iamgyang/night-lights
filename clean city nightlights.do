...
// merge the ADM codes with the actual lat-long data
	import delimited "$hf_input/National Accounts/city_ADM_bridge.csv", colrange(2) clear 
	tempfile adm_codes
	save `adm_codes', replace

	import delimited "$hf_input/National Accounts/oxford_city_data_lat_long.csv", clear 
...
// 	13 cities were not matched because they lied on the EDGE of an object ID polygon
	keep if _m == 3
	drop _m
	save "$hf_input/city_ntl_merge_long.dta", replace
...
// 	get the change in sum of pixel by object ID and year
	...
	// now we have a dataset of annual change in nightlights (but, we have it for 
	// different months)
	// the 'year' variable thus far has been referring to the END year of the change 
	// in nightlights (i.e. year 2013 refers to the change in NL from 2012-2013). 
		rename year yrend
	...
	// So, the change in NL from 2014-2016 should equal 2015 * 2016 change in NTL
		...
		drop if yrend == 2015
		save "$hf_input/annual_change_ntl_long_`func_var'.dta", replace
	...
	//	merge the datasets:
		...
		save "$hf_input/city_ntl_merge_long_`func_var'.dta", replace
		use "$hf_input/city_ntl_merge_long_`func_var'.dta", clear
		
	// Now that we have the full dataset in long format, we'd like to turn it into 
	// wide format so that we can do some regressions on it. For the change in GDP 
	// per capita, we have the delt_gdppc variable, which is define for years only. 
	// For the change in night lights pixels, we have the delt_sumpix variable, 
	// which is defined for a year and month. This means that we first do the pivot 
	// wider for one variable and then we do it again for the next, while merging 
	// at the end.
		...
		save "$hf_input/city_ntl_merge_wide_`func_var'.dta", replace
